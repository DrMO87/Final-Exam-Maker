  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden animate-fade-in">
        ...
        {/* Alerts */}
        {error && (
          <div className="m-4 p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 font-medium">
            ⚠️ {error}
          </div>
        )}
        {successMsg && (
          <div className="m-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-700 font-medium">
            ✅ {successMsg}
          </div>
        )}
        ...
        <div className="p-5 max-h-[70vh] overflow-y-auto">
          ...
          {/* Add User Form */}
          {showAddForm && (
            <form onSubmit={handleCreateUser} className="mb-6 p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
              <h4 className="font-bold text-xs text-[#0B1E36] uppercase tracking-wider">Create New Authorized User</h4>
                    ...
                    <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          ...
                          u.role === 'Control Committee' ? 'bg-amber-100 text-amber-900 border border-amber-200' :
